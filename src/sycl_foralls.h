...
////////////////////////////////////////////////////////////////////////////////
#ifndef __SYCL_FORALLS_H__
#define __SYCL_FORALLS_H__
#include <fstream>
#include <iomanip>
#include <iostream>
#include <map>
...
extern sycl::queue* qu;
extern sycl::event currentEvent;

void insertEvent(sycl::event &event);
float timeEvent(sycl::event &start, sycl::event &stop);

std::vector<int> factors(int N);
std::vector<int> factors(int N, int start);
#ifdef ENABLE_SYCL
template <typename Func>
void forallkernel(int start, int N, Func f, sycl::nd_item<1> item) {
  int tid = start + item.get_global_id(0);
  if (tid < N) f(tid);
}
/*
...
void forallasync(int start, int end, LoopBody &&body) {
  ...
  int grid = blocks * tpb;
  // printf("Launching the kernel blocks= %d tpb= %d \n",blocks,tpb);
  currentEvent = qu->submit([&](sycl::handler& h) {
    h.parallel_for(sycl::nd_range<1>(grid, tpb),
                   [=] (sycl::nd_item<1> item) {

      forallkernel(start, end, body, item);
...
void forall3kernel(const int start0, const int N0, const int start1,
                   ...
                   Func f, sycl::nd_item<3> item) {
  int tid0 = start0 + item.get_global_id(0);
  int tid1 = start1 + item.get_global_id(1);
  int tid2 = start2 + item.get_global_id(2);
...
void forall3kernel(Tag t, const int start0, const int N0,
                   ...
                   const int N2, Func f, sycl::nd_item<3> item) {
  int tid0 = start0 + item.get_global_id(0);
  int tid1 = start1 + item.get_global_id(1);
  int tid2 = start2 + item.get_global_id(2);
...
template <typename Func>
void forall3gskernel(int start0, int N0, int start1, int N1,
                     int start2, int N2, Func f, sycl::nd_item<3> item) {
  for (int i = start0 + item.get_global_id(0); i < N0;
       i += item.get_local_range(0) * item.get_group_range(0))
    for (int j = start1 + item.get_global_id(1); j < N1;
         j += item.get_local_range(1) * item.get_group_range(1))
      for (int k = start2 + item.get_global_id(2); k < N2;
           k += item.get_local_range(2) * item.get_group_range(2))
        f(i, j, k);
}
/*
template <typename LoopBody>
...
void forall3asyncnotimer(T1 &irange, T2 &jrange, T3 &krange, LoopBody &&body) {
  // BRIAN: TODO, remove template and port call
  forall3async<213>(irange, jrange, krange, body);
}
  

template <typename T1, typename T2, typename T3, typename LoopBody>
void forall3GSasync(T1 &irange, T2 &jrange, T3 &krange, LoopBody &&body) {
  sycl::range<3> tpb(irange.tpb, jrange.tpb, krange.tpb);
  sycl::range<3> grid(irange.blocks * irange.tpb,
                      jrange.blocks * jrange.tpb,
                      krange.blocks * krange.tpb);
  
  currentEvent = qu->submit([&](sycl::handler& h) {
    h.parallel_for(sycl::nd_range<3>(grid, tpb),
                   [=] (sycl::nd_item<3> item) {

      forall3gskernel(irange.start, irange.end, jrange.start,
                      jrange.end, krange.start, krange.end, body, item);
...
__global__ void forall2kernel(const int start0, const int N0, const int start1,
                              const int N1, Func f) {
  int tid0 = start0 + get_global_id(0)
  int tid1 = start1 + get_global_id(1)
...
__global__ void forallgskernel(T start, T N, Func f) {
  for (T i = start + get_global_id(0) i < N;
...
void forall3async(T1 &irange, T2 &jrange, T3 &krange, LoopBody &&body) {
  ...
  currentEvent = qu->submit([&](sycl::handler& h) {
    h.parallel_for(sycl::nd_range<3>(grid, tpb),
      ...
      forall3kernel<N>(irange.start, irange.end, jrange.start,
                       jrange.end, krange.start, krange.end, body, item);
...
void forall3async(Tag &t, T1 &irange, T2 &jrange, T3 &krange, LoopBody &&body) {
  ...
  currentEvent = qu->submit([&](sycl::handler& h) {
    h.parallel_for(sycl::nd_range<3>(grid, tpb),
      ...
      forall3kernel<N>(t, irange.start, irange.end, jrange.start,
                       jrange.end, krange.start, krange.end, body, item);
